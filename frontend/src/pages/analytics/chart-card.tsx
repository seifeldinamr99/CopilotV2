import { formatCompactCurrency, truncateLabel } from "@/pages/analytics/utils";
...
export function ChartCard({ title, subtitle, labels, values, format, emptyState }: Props) {
            ...
            {values.map((value, index) => (
              <div key={`${title}-${index}`} className="flex h-full flex-1 flex-col justify-end">
                <div className="relative flex h-full items-end">
                  <div
                    className="w-full rounded-full bg-gradient-to-t from-violet-500/20 via-violet-400/70 to-sky-300/80"
                ...
                <div className="text-center text-[11px] text-white/70">
                  {format === "currency" ? formatCompactCurrency(value) : Math.round(value)}
                </div>
              </div>
            ))}
          </div>
